...
def getDayOfTheMonth(daystart, tasksLenght, i):
    i = daystart + i
    day = i%tasksLenght - 1
    week = int(i/tasksLenght) 
    return week, day
...
def evaluateFreeDays(daystart, tasksLenght, index, person, nGenes):
    if DAYSOFF[person]:
        if isTaskOnFreeDay(daystart, tasksLenght, index, person): # If the person not free
            return -1/nGenes
    return 1/nGenes

# If the person assigned to the task is free that day will return true, false if not
def isTaskOnFreeDay(daystart, tasksLenght, index, person):
    week, day = getDayOfTheMonth(daystart, tasksLenght, index)
    if DAYSOFF[person]:
...
def getTaskByIndex(currentCalendar, tasksLenght, tasksGeneral, index):
    index += getAddition(tasksGeneral.items(), currentCalendar)
    i = index%tasksLenght
