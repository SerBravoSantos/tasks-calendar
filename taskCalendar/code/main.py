...
def testConfiguration(filename, config, tasksWeek):
    ...
    printChromosome(os.path.join(textPath, "calendars/calendar_{}.txt".format(filename, ntests)), ga.best.info, calendarStarts, currentCalendar, tasksLenght, tasksWeek, YEAR, MONTH)
...
if __name__ == "__main__":
    ...
    # tasksWeek2 = {MONDAY: [TOB, CK], TUESDAY: [TIB],  WEDNESDAY: [CBB], THURSDAY:[CK], FRIDAY:[], SATURDAY:[], SUNDAY: [VS]}
    tasksWeek2 = {MONDAY: [CTB], TUESDAY: [CTB],  WEDNESDAY: [CTB], THURSDAY:[CTB], FRIDAY:[CTB], SATURDAY:[CTB], SUNDAY: [CTB]}
    tasksList = [tasksWeek1, tasksWeek2]
    ...
    for taskWeekCalendarNumber, tasksWeek in enumerate(tasksList):
        ...
        peopleChosen = people[[ALBERTO, OSCAR, ANDRIU, ZARRA, AYDEN, BISWU, CEREN, HATICE, SEDA, EMMA, ALAN, DAVOR, SERGIO] ]
        ...
        if test3:
            for configName, parameters in configMutationTest2.items():
                testConfiguration(str(taskWeekCalendarNumber)+'_'+configName, parameters, tasksWeek)
        if testFinal:
            testConfiguration(str(taskWeekCalendarNumber)+'_'+configFinal['name'], configFinal['parameters'], tasksWeek)