...
class Chromosome:
    ...
    def evaluate(self):
        ...
        maxPeopleDistribution = math.sqrt(len(self.people)) * int(averageTasks) * len(self.info)/averageTasks
        # maxPeopleDistribution = 0
        # for person in self.people:
        #     if person in peopleCount.keys():
        #         maxPeopleDistribution += (peopleCount[person] - 1) * len(self.info)//averageTasks
        ...
        for person in self.people:
            ...
            if person in peopleCount.keys():
                if averageTasks - epsilon <= peopleCount[person] <= averageTasks + epsilon:
                    ...
                    tasksPerPersonFitness -= abs(peopleCount[person] - averageTasks)**2/len(self.info)
            else: # If that person is not participating then is doing 0 tasks
                tasksPerPersonFitness -= averageTasks**2/len(self.info)
        ...
        # self.fitness = self.fitness1 * 0.45 + self.fitness2 * 0.35 + self.fitness3 * 0.15 + self.fitness4 * 0.20 + self.fitness5 * 0.05
